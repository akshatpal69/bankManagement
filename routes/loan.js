...
const {loanIR} = require('../ratesAndCharges')
...
router.post('/createLoan', async (req, res) => {
    ...
    //         let e = result[0].fnamecustomer
            return res.status(201).json({ a:'createLoanRoute' })
    //       });
...
router.post('/submitEMI', async (req, res) => {
    const { }  = req.body
...
router.post('/finishLoan', async (req, res) => {
    const { }  = req.body
