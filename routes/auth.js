...
router.post("/login", async (req, res) => {
    let { empname, password, empClientIP } = req.body
    if(!empname&&!password&&!empClientIP){
        ...
        connection.query(checkCredentialsQuery, function (err, result) {
            ...
            if (result.length > 0) {
                if (password !== result[0].password) {
                    ...
                    let saveSessionQuery = `INSERT INTO empsessions (sessionid, empid, empClientIP) VALUES ("${sessionid}", "${result[0].empid}", "${empClientIP}")`
                    connection.query(saveSessionQuery, (err, result) => {
                        if (err) return res.status(400).json({ response:err })
                    })
                     return res.status(200).json({ response:'loggedIn', name:result[0].name})
                }
            } else {
